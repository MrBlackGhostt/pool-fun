...
pub struct BuyToken<'info> {
    ...
    #[account(mut, seeds=[b"bonding-pump", mint.key().as_ref()] ,bump ) ]
    pub curve_config: Account<'info, CurveConfiguration>,

    #[account(mut,  seeds = [b"bonding-pump-mint", mint_creator.key().as_ref(),name.as_bytes()], 
        bump                                               )]
...
impl<'info> BuyToken<'info> {
    pub fn buy_token(&mut self, mut amount_in: u64, bump: u8, min_token_out: u64) -> Result<()> {
        ...
        let product = old_sol_reserve
            .checked_mul(old_virtual_token)
            .expect("The prodctuct go out of bound");
        ...
        let signer_seeds = &[b"bonding-pump", mint_key.as_ref(), &[bump]];
        let seed = &[&signer_seeds[..]];

        let cpi_context =
            CpiContext::new(self.token_program.to_account_info(), cpi_accounts).with_signer(seed);
        ...
        let sol_transfer_ctx =
            CpiContext::new(self.system_program.to_account_info(), sol_transfeir_account);
        ...
        if self.curve_config.virtual_sol_reserve >= GRADUATION_THRESHOLD {
            ...
            msg!(
                "🎓 Token graduated! Market cap: {} SOL",
                self.curve_config.virtual_sol_reserve / 1_000_000_000
            );
        }
